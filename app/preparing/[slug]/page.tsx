import { getPrincipleTopic, getAllPrincipleSlugs } from "@/lib/principles";
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  const topic = getPrincipleTopic(params.slug);
  if (!topic) return {};
  return { title: `${topic.title} — Interview Coach` };
}

export default function PrincipleTopicPage({ params }: { params: { slug: string } }) {
  const topic = getPrincipleTopic(params.slug);
  if (!topic) notFound();
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">{topic.questionCount} nguyên tắc</p>
      </div>
